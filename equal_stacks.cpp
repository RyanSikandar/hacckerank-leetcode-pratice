...
{
    //Reversing because the vector is in reversed form
    reverse(h1.begin(),h1.end());
    reverse(h2.begin(),h2.end());
    reverse(h3.begin(),h3.end());
    
    //Summing the vector values
    int sum1=0;
    int sum2=0;
    int sum3=0;
    for(auto i:h1)
    ...
    while(!h1.empty() && !h2.empty() && !h3.empty())//Loop continues till one of the stack is empty
    {
        if(sum1==sum2 && sum2==sum3)//returing statement
        {
            int height= sum1;
            return height;
        }
